...
namespace {
constexpr uint32_t backend_abi_version = 1;
...
{
    assert(error_message != nullptr || error_message_size == 0);
    assert(error_message_size <= std::numeric_limits<size_t>::max());
    if (error_message != nullptr && error_message_size > 0)
        error_message[0] = '\0';
    assert(error_message == nullptr || error_message_size == 0 || error_message[0] == '\0');
    assert(error_message_size == 0 || error_message != nullptr);
}
...
void write_error(char *error_message, size_t error_message_size, const char *message)
{
    ...
    assert(error_message != nullptr || error_message_size == 0);
    if (error_message != nullptr && error_message_size > 0)
        std::snprintf(error_message, error_message_size, "%s", message);
    assert(error_message_size == 0 || error_message != nullptr);
    assert(error_message == nullptr || error_message_size == 0 || error_message[error_message_size - 1] == '\0' || std::strlen(error_message) < error_message_size);
}
...
{
    assert(gpu_name != nullptr || gpu_name_size == 0);
    assert(error_message != nullptr || error_message_size == 0);
    clear_error(error_message, error_message_size);
    if (gpu_name == nullptr || gpu_name_size == 0 || gpu_index < 0)
    {
...
{
    assert(error_message != nullptr || error_message_size == 0);
    assert(backend_out != nullptr || configuration == nullptr || model_directory == nullptr || model_directory_size == 0);
    clear_error(error_message, error_message_size);
    ...
    size_t error_message_size)
{
    ...
    if (output_size < expected_size || width > static_cast<uint32_t>(std::numeric_limits<int>::max()) || height > static_cast<uint32_t>(std::numeric_limits<int>::max()))
    {
        write_error(error_message, error_message_size, "RGB24 output buffer is too small");
        return INTERPOLATE_BACKEND_STATUS_INVALID_ARGUMENT;
    }
